  ...
  options: [
    {
      ...
      description: "Elige el juego para buscar partida",
      required: true,
      choices: [
    ...
    for (const [queueGame, queues] of Object.entries(gameQueues)) {
      for (const queue of queues) {
        if (queue.some(player => player.id === userId)) {
          return interaction.reply({ content: `Ya estás en una cola de ${queueGame}. Usa /salir-cola para salir primero.`, ephemeral: true });
        }
      }
    }
