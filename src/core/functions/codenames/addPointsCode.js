...
async function addPointsCode(client, userId, points) {
  try {
    const user = await CodenamesUserModel.findOne({ id: userId });
    if (!user) {
      
      const newUser = new CodenamesUserModel({
        id: userId,
